...
import type { PlainRemoteFs } from "./fsPlain";
import {
  type InternalDBs,
...
export const doActualSync = async (
  ...
  for (let i = 0; i < nested.length; ++i) {
    ...
    for (let j = 0; j < operations.length; ++j) {
      ...
      let nextOperation = 0;
      const workerCount = Math.min(Math.max(1, concurrency), singleLevelOps.length);
      const runWorker = async () => {
        while (potentialErrors.length < 3) {
          const operationIndex = nextOperation;
          nextOperation += 1;
          if (operationIndex >= singleLevelOps.length) return;
          const val = singleLevelOps[operationIndex];
          const key = val.key;
          try {
          // console.debug(
          ...
          // console.debug(`finished ${key}`);
            ...
            potentialErrors.push(new Error(`${key}: ${message}`));
          }
        }
      };
      ...
      if (potentialErrors.length > 0) {
        if (potentialErrors.length >= 3 && nextOperation < singleLevelOps.length) {
          potentialErrors.push(
            new Error("too many errors, stop the remaining tasks")
          );
        }
        throw new SyncBatchError(potentialErrors);
      }
    }
