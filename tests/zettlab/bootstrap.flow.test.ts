...
describe("PC bootstrap flow", () => {
  it("accepts only a loopback token claim and writes the fixed WebDAV defaults", () => {
    const claim = buildBootstrapClaimUrl({ token: "a".repeat(43), port: "43123" });
    assert.equal(claim, `http://127.0.0.1:43123/claim?token=${"a".repeat(43)}`);
    assert.equal(buildBootstrapClaimUrl({ token: "short", port: "43123" }), null);
    assert.equal(buildBootstrapClaimUrl({ token: "a".repeat(43), port: "0" }), null);
    assert.equal(
      buildBootstrapCompletionUrl({ token: "a".repeat(43), port: "43123" }, "ok"),
      `http://127.0.0.1:43123/complete?token=${"a".repeat(43)}&status=ok`
    );

    const normalized = normalizeBootstrapPayload(payload);
    assert.deepEqual(normalized, payload);
    assert.equal(normalizeBootstrapPayload({ ...payload, address: "file:///tmp/vault" }), null);
    assert.equal(normalizeBootstrapPayload({ ...payload, username: "admin" }), null);
...
describe("mobile bootstrap flow", () => {
  ...
  it("accepts a direct owned-HTTPS hand-off from the mobile App", () => {
    ...
    assert.equal(buildBootstrapClaimRequest({ mode: "direct", token, port: "43123" }), null);
    assert.equal(
      normalizeDirectBootstrapPayload({
        mode: "direct",
